...
local GetDamage = function(spell, target)
	...
	local missing_health = (target.AsAttackableUnit.Health * 100) / target.AsAttackableUnit.MaxHealth
   --print(missing_health)
   if slot == _R and IsReady(R) then
		local flatAD = Player.TotalAD
        local RRawDamage = { 100, 150, 200 }
		if IsValidTarget(target) then
			R_damages = (RRawDamage[level] + (0.6 * flatAD)) + ((2.667/100) * missing_health * (RRawDamage[level] + (0.6 * flatAD)))
			--print(R_damages)
		end
		return R_damages
	end
...
Menu.RegisterMenu("XX Riven", "XX Riven", function()
    ...
    Menu.NewTree("R", "[R] Blade of the Exile", function()
        Menu.NewTree("ComboR", "Combo Options", function()
            Menu.Checkbox("ComboUseR", "Enabled (Activating Again when killable)", true)
...
if IsReady(R) and GetMenuValue("ComboUseR") then
			...
			for i, hero in pairs(heroes) do
				...
				if hero and IsValidTarget(hero, R.Range) then
					local predi = Prediction.GetPredictedPosition(hero, R, Player.Position)
					local dist = Player.Position:Distance(hero.Position)
					local damage = GetDamage(R, hero)		
					local true_damage = DamageLib.CalculatePhysicalDamage(Player, hero, damage)
					if predi and true_damage > hero.AsAttackableUnit.Health and dist <= R.Range  and predi.HitChance > (GetMenuValue("RHitchance")/100) then		
        ...
        for k, hero in pairs(heroes) do
            ...
            if hero.IsVisible and hero.IsOnScreen and not hero.IsDead then
                ...
                if true_damage >= (hero.Health + hero.ShieldAll) then
					Renderer.DrawFilledRect(Vector(hpBarPos.x + position - 45, hpBarPos.y - 23), Vector(value, 11), 1, 0xADFF2FFF)
				else
